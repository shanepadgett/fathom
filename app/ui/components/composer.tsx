...
export function Composer(
  ...
  async function attach(selected: FileList | File[] | null) {
    ...
    if (attaching() || sending() || media.busy()) {
      setAttachmentError(
        "Wait for the current attachment or send to finish, then try again.",
      );
      return;
    }
    ...
    try {
      ...
      for (const file of Array.from(selected)) {
        ...
        if (file.size > 1024 * 1024) {
          throw new Error(
            `${file.name} exceeds the 1 MB text attachment limit.`,
          );
        }
        const bytes = new Uint8Array(await file.arrayBuffer());
        if (bytes.includes(0)) {
          throw new Error(
            `${file.name} is a binary file. Text context is supported here.`,
          );
        }
      ...
      updateDraft(
        targetSession,
        (value) => [value, ...contexts].filter(Boolean).join("\n\n"),
      );
    } catch (error) {
  ...
  const levels = () =>
    props.models.find((model) =>
      model.provider === props.state?.session.provider &&
      model.id === props.state?.session.model
    )?.thinkingLevels ?? [];
  const ready = () =>
    !!(text().trim() || media.assets().length) && !sending() && !attaching() &&
    !media.busy() && !media.error();
  async function send(mode?: "steer" | "follow_up") {
    if (!sessionId() || !ready()) return;
    const targetSession = sessionId(), submitted = text();
    setSending(true);
    try {
      await props.send(
    ...
    >
      ...
      <div class="pointer-events-auto relative mx-auto w-full max-w-transcript px-6">
        ...
        >
          ...
          />
          <Show
            when={attachmentError() || drafts.error(sessionId()) ||
              media.error()}
          >
          ...
          <Show when={media.error()}>
            <Button onClick={() => void media.refresh()}>
              Retry attachments
            </Button>
          </Show>
          <textarea
          ...
          <div class="flex flex-wrap items-center justify-between gap-3 text-sm">
            <Button
              disabled={attaching() || sending() || media.busy() ||
                !sessionId()}
              onClick={() => files.click()}
            >
              <Icon name="plus" />
            ...
            <span class="ml-auto flex flex-wrap items-center gap-3">
              ...
              <Show
                ...
                fallback={
                  <>
                    <Show
                      when={["error", "interrupted"].includes(
                        props.state?.session.status ?? "",
                      )}
                    >
