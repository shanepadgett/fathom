...
function buttonClass(props: ButtonStyle) {
  ...
  const dimensions = size === "normal"
    ...
    : `min-h-0 py-1 text-sm font-normal ${
      variant === "quiet" ? "px-0" : "px-3"
    }`;
...
export function Button(
  props: JSX.ButtonHTMLAttributes<HTMLButtonElement> & ButtonStyle,
) {
...
export function LinkButton(
  props: JSX.AnchorHTMLAttributes<HTMLAnchorElement> & ButtonStyle,
) {
  const [local, native] = splitProps(props, ["variant", "size", "class"]);
...
export function Modal(
  ...
  return (
    ...
    >
      ...
      <div class={`dialog-content ${props.contentClass ?? ""}`}>
        {props.children}
      </div>
    </dialog>
  );
...
export function Field(
  props: { label: string; hint?: string; children: JSX.Element },
) {
  return (
    <label class="field">
