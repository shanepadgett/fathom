...
export function EditorStatus(props: {
  ...
  return (
    <footer class="flex shrink-0 items-center justify-between gap-3 border-t border-line px-4 py-2 text-xs text-muted">
      <span title={props.server?.error}>
        Problems{" "}
        <Show
          when={props.server && !props.server.running && props.server.name !== "No language server"}
        >
      ...
      <div class="flex items-center gap-3">
        {props.children}
        <Show when={props.position} fallback={<span>Select a file to edit</span>}>
          {(position) => (
            <span>
              Ln {position().line}, Col {position().column} · UTF-8 · {position().language}
            </span>
          )}
        </Show>
      </div>
