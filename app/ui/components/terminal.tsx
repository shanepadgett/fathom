...
export function TerminalPanel(
  ...
  const [terminals] = createResource(
    ...
    async (source) => {
      ...
      const selected = available.find((item) => item.id === active()) ??
        available.find((item) => item.running) ?? available[0];
  ...
  const start = () => {
    if (
      !ready() || terminals.loading || terminals.error || !terminals() ||
      starting() || !props.sessionId.trim()
    ) return;
    automaticStart = false;
    setStarting(true);
    ...
    return act(async () => {
      try {
        const created = await props.transport.request<TerminalState>(
          "terminal.start",
          { sessionId },
        );
        if (
          !disposed && current === environment && sessionId === props.sessionId
        ) select(created);
      } finally {
        setStarting(false);
      }
  ...
  createEffect(() => {
    if (
      ready() && !terminals.loading && !terminals.error && terminals() &&
      !active() && !starting() && automaticStart
    ) void start();
  });
  createEffect(() => {
    props.theme;
  ...
  onMount(() => {
    void Promise.all([
      ...
      terminal.onData((text) => {
        if (terminals()?.some((item) => item.id === active() && item.running)) {
          void act(() =>
            props.transport.request("terminal.write", { id: active(), text })
          );
        }
      });
    ...
    const unsubscribe = props.transport.onEvent((event) => {
      ...
      if (
        event.type === "disconnected" || event.type === "connected" ||
  ...
  return (
    <section class="flex h-64 min-h-32 resize-y flex-col border-t border-line bg-canvas">
      <header class="flex h-9 shrink-0 items-center gap-2 border-b border-line bg-surface px-3 text-sm">
        <span>Terminal</span>
        <For
          each={terminals()?.filter((item) =>
            item.sessionId === props.sessionId
          )}
        >
          {(item) => (
            <Button
              classList={{ selected: item.id === active() }}
              onClick={() => select(item)}
            >
        ...
        <span class="spacer" />
        <Button
          disabled={!ready() || terminals.loading || !!terminals.error ||
            starting()}
          onClick={() => void start()}
          title="New terminal"
        >
        ...
        <Button
          disabled={!terminals()?.some((item) =>
            item.id === active() && item.running
          )}
          onClick={() =>
            void act(async () => {
