...
export function HtmlPreview(props: {
    ...
    frame?.contentWindow?.postMessage(
      {
        channel,
        mode: selecting(),
        color: getComputedStyle(document.documentElement).getPropertyValue("--color-focus").trim(),
      },
      "*",
    );
  createEffect(sync);
  ...
  return (
    <div class="space-y-3">
      <div class="flex flex-wrap gap-3" role="group" aria-label="Preview feedback selection">
        <Button
          variant="secondary"
          aria-pressed={selecting() === "element"}
          onClick={() => setSelecting(selecting() === "element" ? undefined : "element")}
        >
          {selecting() === "element" ? "Cancel element selection" : "Select element"}
        </Button>
        <Button
          variant="secondary"
          aria-pressed={selecting() === "area"}
          onClick={() => setSelecting(selecting() === "area" ? undefined : "area")}
        >
