...
export function BrowserPanel(
  ...
  const [drag, setDrag] = createSignal<
    { x: number; y: number; endX: number; endY: number; pointer: number }
  >();
  ...
  const [selection, setSelection] = createSignal<
    { x: number; y: number; url: string; bounds?: BrowserAnnotationBounds }
  >();
  ...
  const { draft: annotations, refetch } = browserFeedback(
    props.transport,
    () => props.sessionId,
  );
  ...
  onCleanup(props.transport.onEvent((event) => {
    ...
    if (
      ...
      (event.data as { viewId?: string } | undefined)?.viewId !==
        pairing.viewId()
    ) return;
  ...
  const point = (event: MouseEvent) => {
    ...
    return {
      x: Math.max(
        ...
        Math.min(
          pageSize().width,
          (event.clientX - box.left) * pageSize().width / box.width,
        ),
      ),
      y: Math.max(
        0,
        Math.min(
          pageSize().height,
          (event.clientY - box.top) * pageSize().height / box.height,
        ),
      ),
    };
  };
      ...
      <div class="browser-scroll">
        <div
          ...
          onPointerMove={(event) => {
            ...
            setDrag((value) =>
              value && ({ ...value, endX: end.x, endY: end.y })
            );
          }}
          onPointerUp={(event) => {
        ...
        >
          <Show
            ...
            fallback={
              <div class="browser-empty">
                ...
                <p>
                  Open a local development server or website. Select Annotate to
                  collect feedback, then send it as one message.
                </p>
              </div>
            }
          >
            ...
            {(annotation, index) => (
              ...
              >
                ...
                <span
                  ...
                  style={{
                    left: `${
                      (annotation.x + offset(annotation).x) / pageSize().width *
                      100
                    }%`,
                    top: `${
                      (annotation.y + offset(annotation).y) /
                      pageSize().height * 100
                    }%`,
                  }}
                >
          ...
          <Show when={dragBounds() ?? selection()?.bounds}>
            {(bounds) => (
              <AnnotationArea bounds={bounds()} viewport={pageSize()} />
            )}
          </Show>
        </div>
      </div>
      <Show when={selection()}>
        ...
        >
          <p class="text-xs text-muted">
            Add feedback for the selected{" "}
            {selection()?.bounds ? "area" : "point"}. Stage comments, then send
            them together.
          </p>
          <CommentField
            value={comment()}
            ...
            <Button
              ...
              onClick={() =>
                void act(async () => {
                  ...
                  const selected = selection()!, sessionId = props.sessionId;
