...
export function CommitPlanEditor(props: {
  ...
  const update = (index: number, changes: Partial<CommitGroup>) =>
    change((items) => items.map((item, i) => (i === index ? { ...item, ...changes } : item)));
    ...
    <Index each={props.commits}>
      {(commit, index) => (
        <article
          ...
          onDragStart={(event) =>
            event.dataTransfer?.setData("application/x-fathom-commit-index", String(index))
          }
          onDragOver={(event) => event.preventDefault()}
          onDrop={(event) => {
            event.preventDefault();
            const raw = event.dataTransfer?.getData("application/x-fathom-commit-index");
            if (!raw) return;
            const from = Number(raw);
            if (Number.isInteger(from) && from >= 0 && from < props.commits.length)
              move(from, index);
          }}
        >
          <header>
            ...
            />
            <Button disabled={props.disabled || index === 0} onClick={() => move(index, index - 1)}>
              ↑
            </Button>
            <Button
          ...
          <textarea
            ...
            value={commit().body}
            onInput={(event) => update(index, { body: event.currentTarget.value })}
          />
          ...
          <div class="actions">
            ...
            <Button
              disabled={props.disabled}
              onClick={() => change((items) => items.filter((_, i) => i !== index))}
            >
              Exclude commit
            </Button>
          </div>
