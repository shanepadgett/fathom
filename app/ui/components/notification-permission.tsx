...
export function NotificationPermissionControl() {
  const [permission, { refetch }] = createResource(() => notificationPermission());
  ...
  async function act(preview: boolean) {
    ...
    try {
      if (preview) {
        await notify("Fathom notifications", "Desktop notifications are enabled.", {
  ...
  return (
    <div class="mb-4 text-sm">
      ...
      <Button
        ...
        onClick={() => void act(!permission.error && permission() === "granted")}
      >
        {busy()
          ? "Working…"
