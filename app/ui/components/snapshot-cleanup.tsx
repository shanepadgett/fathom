...
export function SnapshotCleanup(
  props: { transport: Transport; onCleaned: () => void },
) {
  ...
  async function inspect() {
    ...
    try {
      setPlan(
        await props.transport.request<SnapshotPrunePlan>(
          "snapshots.planPrune",
          { projectId },
        ),
      );
  ...
  async function clean() {
    ...
    setError("");
    try {
      const value = await props.transport.request<{ removed: number }>(
        "snapshots.prune",
        { projectId, trees: current.trees },
      );
      setPlan(undefined);
      setResult(
  ...
  return (
    <section class="space-y-3 border-t border-line pt-4">
      <div>
        ...
        <p class="muted">
          Current project only. Remove snapshots unused for 30 days.
        </p>
      </div>
      <Show when={error()}>
      ...
      <Show when={result()}>
        <p role="status" class="text-sm text-muted">{result()}</p>
      </Show>
      <Show
        when={plan()}
        fallback={
      ...
      >
        {(value) => (
          <>
            <p>
              {value().trees.length} snapshots eligible; {value().retained}{" "}
              retained.
            </p>
            <p class="text-sm text-muted">
              Removing snapshots can permanently remove file restoration for
              older messages. Conversation history remains available. Recently
              reused snapshots are checked again and kept.
            </p>
            <p class="text-xs text-muted">
              Snapshots without age records receive a new 30-day retention
              period. Cleanup also collects objects left by an interrupted
              cleanup.
            </p>
