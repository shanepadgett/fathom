...
export async function discover(
  ...
  for (const base of roots) {
    const directory = base === home ? join(home, "plugins") : join(base, ".fathom/plugins");
  ...
  for (const path of paths) {
    ...
    const compiled = await compileBackend(path, join(cacheDirectory, key));
    ...
    if (!manifest || manifest.apiVersion !== 1 || typeof manifest.id !== "string")
    ...
    if (manifest.frontend) {
      assets.set(
        manifest.id,
        typeof manifest.frontend === "string" ? resolve(dirname(path), manifest.frontend) : path,
      );
    }
  }
  return {
