...
export function artifactsOnBranch(entries: Entry[]): Artifact[] {
  const artifacts = entries.filter((entry) => entry.custom?.type === "artifact")
    .map((entry) => entry.custom!.data as Artifact);
  const superseded = new Set(artifacts.map((artifact) => artifact.supersedes));
  const approved = new Set(
    entries.filter((entry) => entry.custom?.type === "artifact-approval")
      .map((entry) =>
        (entry.custom!.data as { artifactId: string }).artifactId
      ),
  );
