...
export default definePlugin({
  ...
  backend: {
    ...
    activate(ctx) {
      ...
      index.exec(
        "CREATE VIRTUAL TABLE tool_index USING fts5(name, description, tags)",
      );
      ...
      const allowed = (sessionId: string, tool: ToolDefinition) => {
        ...
        return policy !== "no-terminal" ||
          !["bash", "pty", "script"].includes(tool.name);
      };
      const registry: ToolRegistry = {
        register(tool) {
          ...
          index.prepare(
            "INSERT INTO tool_index(name, description, tags) VALUES (?,?,?)",
        ...
        list: (sessionId, includeDeferred = false) =>
          [...tools.values()].filter((tool) =>
            ...
            (includeDeferred || !tool.deferred ||
              activated.get(sessionId)?.has(tool.name))
          ),
        search(sessionId, query) {
          ...
          const matches = rows.map((row) => tools.get(String(row.name))!)
            .filter((tool) => allowed(sessionId, tool)).slice(0, 8);
      ...
      registry.register({
        ...
        execute: (args, context) =>
          Promise.resolve(
            JSON.stringify(
              registry.search(context.sessionId, String(args.query)).map((
