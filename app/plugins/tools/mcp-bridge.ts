...
function typeOf(value: unknown): string {
  ...
  if (schema.type === "object") {
    ...
    return `{${Object.entries((schema.properties ?? {}) as Record<string, Record<string, unknown>>)
      .map(
        ([key, value]) =>
          `${JSON.stringify(key)}${required.includes(key) ? "" : "?"}: ${typeOf(value)}`,
      )
      .join("; ")}}`;
  }
  return "unknown";
...
export async function mcpBridge(directory: string, registry: ToolRegistry, context: ToolContext) {
  const token = crypto.randomUUID();
  const tools = registry
    .list(context.sessionId, true)
      ...
      .map(
        ([id, tools]) =>
          `export const ${id} = {${tools
            .map(
              (tool) =>
                `${JSON.stringify(tool.name.slice(`mcp_${id}_`.length))}: (args: ${typeOf(
                  tool.parameters,
  ...
  await atomicWrite(config, JSON.stringify({ imports: { "fathom:mcp": "./mcp.ts" } }));
