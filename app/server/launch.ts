...
export async function restoreDesktopLaunch(): Promise<boolean> {
  const native =
    typeof (Deno as typeof Deno & { BrowserWindow?: unknown }).BrowserWindow ===
      "function";
  if (!native) return false;
  const userHome = Deno.env.get("HOME");
  if (!userHome || Deno.build.os !== "darwin") return true;
  const id = createHash("sha256").update(Deno.execPath()).digest("hex").slice(
    0,
    24,
  );
  const path = join(
  ...
  try {
    ...
    const sameProfile = explicitHome === undefined ||
      explicitHome === saved.FATHOM_HOME;
    const next: Record<string, string> = {};
    for (const key of PATH_KEYS) {
      const explicit = Deno.env.get(key);
      const value = explicit ??
        (sameProfile && typeof saved[key] === "string"
          ? saved[key]
          : undefined);
