...
The UI source lives here; Vite bundles it into `ui/` while preserving public
plugin URLs. The shell, conversation, composition panel, and context meter use
Solid JSX and reactive stores. The transport and plugin contracts remain
unchanged. `main.js` imports only the plugins declared by backend `uiPlugins`,
in configured order. No UI is hardcoded into the loader except startup failure
and retry. A module exports
