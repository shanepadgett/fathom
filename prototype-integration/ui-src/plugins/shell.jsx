    ...
    <>
      ...
      <div class="workbench">
        <main class="main-view">
          <For each={views().filter((view) => view.slot === "main")}>
        ...
        <button
          ...
          onPointerDown={(event) => {
            ...
            node.onpointermove = (e) => {
              ...
              const percent = Math.max(
                25,
                Math.min(65, ((e.clientX - bounds.left) / bounds.width) * 100),
              );
              node.parentElement.style.setProperty("--conversation-width", `${percent}%`);
            };
            node.onpointerup = () => {
          ...
          onKeyDown={(event) => {
            ...
            const current = parseFloat(parent.style.getPropertyValue("--conversation-width")) || 40;
            parent.style.setProperty(
              "--conversation-width",
              `${Math.max(25, Math.min(65, current + (event.key === "ArrowRight" ? 2 : -2)))}%`,
            );
          }}
        />
        <section class="editor-pane">
          <For each={views().filter((view) => view.slot === "editor")}>
            {(view) => <View view={view} host={host} />}
          </For>
        </section>
