...
try {
  ...
  async function job(name: string, args: Record<string, unknown>, approve = false) {
  ...
  await check(
    !issues.pending && issues.diagnostics.some((d: { code: number }) => d.code === 2322),
  ...
  await check(
    (await job("mcp_greet", { name: "Fathom" }, true)).includes("Hello, Fathom!"),
  ...
  await check(
    (
      await job(
        "script",
        {
          code: 'import { mcp } from "fathom:mcp"; console.log(await mcp.greet({name:"Script"}));',
        },
        true,
      )
    ).includes("Hello, Script!"),
    "approved Deno script calls MCP through server bridge",
  );
  remote = Deno.serve({ hostname: "127.0.0.1", port: 0, onListen() {} }, mcpHttp.fetch);
  ...
  await check(
    (await controller.workbenchState()).jobs.find((j) => j.id === denied.id)?.status === "error",
  ...
  controller.send("# Saved conversation\n\n```typescript\nconst answer = 42;\n```");
  ...
  server = Deno.serve({ hostname: "127.0.0.1", port: 0, onListen() {} }, createHandler(controller));
  ...
  await page.waitForFunction(
    () => document.querySelector(".monaco-container")?.getAttribute("data-marker-count") === "0",
  );
  await check(
  ...
  await page.waitForFunction(
    () =>
      Number(document.querySelector(".monaco-container")?.getAttribute("data-marker-count")) > 0,
  );
  ...
  await check(!errors.length, `browser has no uncaught errors: ${errors.join(", ")}`);
  await page.screenshot({
    path: "/tmp/fathom-integration.png",
    fullPage: true,
  });
  console.log("PASS: editor markers clear after agent fix; screenshot /tmp/fathom-integration.png");
} finally {
  await browser?.close();
  await controller.dispose();
