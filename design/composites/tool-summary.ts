...
export class ToolSummaryElement extends DesignElement {
  ...
  override render() {
    const counts = (["read", "edited", "written"] as const).map((action) => ({
      action,
      files: [...new Set(this.operations.filter((operation) => operation.action === action).flatMap((operation) => operation.files))],
    ...
    return html`<details class="group text-sm text-muted">
      <summary class="flex items-baseline gap-2 py-2">
        ...
        <span>${counts.map(({ action, files }) => `${files.length} ${files.length === 1 ? "file" : "files"} ${action}`).join(" · ") || "No file operations yet"}</span>
      </summary>
      <div class="ml-4 space-y-3 border-l border-line py-2 pl-4">
        ${counts.map(({ action, files }) => html`<div><p class="mb-1 capitalize">${action}</p><ul class="space-y-1 font-mono">${files.map((file) => html`<li class="break-words">${file}</li>`)}</ul></div>`)}
      </div>
    </details>`;
  }
}
