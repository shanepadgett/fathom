...
export const chatListItem = (chat: Chat, project: Project, selected = false) =>
  ...
  <div class="flex items-center justify-between gap-1.5 text-micro text-muted">${projectIdentity(
    project.name,
  ...
  <div class="mt-1 flex min-w-0 items-center justify-between gap-1.5 text-micro text-muted">${branchIdentity(
    chat.branch,
  )}${chat.status ? statusDot("action", chat.status, true) : ""}</div>
</div>`;
export const chatList = (chats: Chat[], projects: Project[], selected: string) =>
...
export const sessionSidebar = (chats: Chat[], projects: Project[], selected: string) =>
  `<div data-component="session-sidebar">
  <div class="flex h-10 items-center gap-2 px-3"><div class="min-w-0 flex-1">${button({
    label: "Search chats",
