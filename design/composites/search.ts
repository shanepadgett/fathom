...
export const keyboardHintBar = (action: string) =>
  `<footer class="flex h-9 items-center gap-4 border-t border-line bg-surface px-4 text-micro text-muted">${shortcutHint(
    ["↑", "↓"],
    "Navigate",
  )}${shortcutHint(["Enter"], action)}${shortcutHint(["Esc"], "Close")}</footer>`;
export const searchSurface = (
  ...
  `<section data-component="search-surface" class="w-search-panel overflow-hidden rounded-lg border border-line bg-canvas shadow-md" aria-label="${text(
    label,
  )}">${field}<div class="px-2 pb-2"><h3 class="px-3 py-2 text-xs text-muted">${text(
    heading,
  )}</h3>${results}</div>${keyboardHintBar(action)}</section>`;
const resultFrame = (leading: string, body: string, trailing: string, selected: boolean) =>
...
export const projectSearchResult = (project: Project, shortcut: string, selected = false) =>
  resultFrame(
    `<span class="text-muted">${icon("folder", "large")}</span>`,
    `<p class="truncate text-sm">${text(
      project.name,
    )}</p><p class="mt-0.5 truncate text-xs text-muted">Local · ${text(project.path)}</p>`,
    text(shortcut),
    selected,
  );
export const chatSearchResult = (chat: Chat, project: Project, selected = false) =>
  resultFrame(
    `<span class="text-muted">${icon("chat-circle-text", "large")}</span>`,
    `<p class="truncate text-sm" title="${text(chat.title)}">${text(
      chat.title,
    )}</p><div class="mt-1 text-xs text-muted">${chatMetadata(project.name, chat.branch)}</div>`,
    text(chat.time),
    selected,
  );
export const projectPicker = (projects: Project[]) =>
  searchSurface(
    ...
    projects.length
      ? projects
          .map((project, index) => projectSearchResult(project, `⌘${index + 1}`, index === 0))
          .join("")
