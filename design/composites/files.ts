...
export const fileItem = (file: FileNode, selected = false, tree = false) =>
  `<div data-file-row class="flex h-6 min-w-0 items-center gap-1.5 rounded-sm pr-2 ${
    ...
    icon(file.icon ?? "file-text")
  }</span><span class="min-w-0 flex-1 truncate">${text(file.name)}</span>${
    changeStatus(file.status)
  }</div>`;
export const folderItem = (name: string, expanded: boolean) =>
  `<button type="button" class="flex h-6 w-full min-w-0 items-center gap-1.5 rounded-sm px-1 text-left hover:bg-canvas" aria-expanded="${expanded}"><span class="flex items-center gap-1.5 text-muted">${
    icon(expanded ? "caret-down" : "caret-right", "small")
  }${icon(expanded ? "folder-open" : "folder")}</span><span class="truncate">${
    text(name)
  }</span></button>`;
const treeNode = (
  node: FileNode,
...
export const fileTree = (
  ...
  `<div data-component="file-tree" class="p-2 text-dense leading-none" aria-label="File explorer">${
    nodes.map((node) => treeNode(node, selected, expanded)).join("")
  }</div>`;
export const changedFilesList = (
  ...
  `<div data-component="changed-files" class="p-2 text-dense" aria-label="Changed files"><p class="flex flex-wrap items-center gap-2 px-2 pb-2 text-muted">${files.length} files changed ${
...
export const filesSidebar = (
  ...
  `${
    tabStrip(
      [{ id: "files", label: "Files" }, { id: "changes", label: "Changes" }],
      view,
      "Files and changes",
    )
  }${
