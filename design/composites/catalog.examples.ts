...
import { changeStatus, diffStat, meter, shortcutHint, statusDot } from "../primitives/content.ts";
import { button, iconButton, selectorButton, tabStrip } from "../primitives/controls.ts";
...
export const primitiveEntries = [
  ...
  },
  {
    ...
    examples: [
      example(
        "Changes",
        `<div class="flex items-center gap-3">${(["A", "M", "D", "R", "?"] as const)
          .map(changeStatus)
          .join("")}${diffStat(24, 8)}${statusDot("action", "Reviewing changes")}</div>`,
      ),
      example(
        "Usage",
        `<div class="flex gap-4">${meter(48, 200, "Context usage")}${meter(
          195,
          200,
...
export const compositeEntries = [
  {
    ...
    examples: [
      example(
        "Sidebar",
        `<div class="w-sidebar bg-surface">${sessionSidebar(
          scenario.chats,
          scenario.projects,
      ...
      example(
        "Long title",
        `<div class="w-sidebar bg-surface">${chatListItem(
          longChat,
          scenario.projects[0],
          true,
  ...
  },
  {
    ...
    examples: [
      example(
        "Tree",
        `<div class="w-files-sidebar bg-surface">${fileTree(
          scenario.files,
      ...
      example(
        "Changes",
        `<div class="w-files-sidebar bg-surface">${changedFilesList(
  ...
  },
  {
    ...
    examples: [
      ...
      example(
        "Narrow",
        `<div class="w-files-sidebar">${composer(scenario.model, scenario.reasoning)}</div>`,
      ),
    ],
  },
  {
    ...
    examples: [
      example(
        "Editor",
        `<div class="flex h-workspace-preview">${editorPane(
  ...
  },
  {
    ...
    examples: [
      example("File diff", diffPane(scenario.path, diffLines, scenario.changes.count, 12, 4)),
    ],
  },
  {
    ...
    examples: [
      example(
        "Inspector",
        `<div class="w-sidebar bg-surface">${sessionInspector(scenario.inspector)}</div>`,
      ),
    ],
  },
  {
    ...
    examples: [
      ...
      example(
        "Footer",
        workspaceStatusBar("Reviewing changes", scenario.changes.count, scenario.context),
      ),
    ],
  },
];
