...
import { workspaceHeader, workspaceStatusBar } from "../composites/workspace-chrome.ts";
import {
  sidebarPane,
  workspaceDrawer,
  workspaceOverlay,
...
export type AgentState = "base" | "no-session" | "diff" | "projects" | "chat-search";
...
export const agentWorkspace = (data: WorkspaceScenario, state: AgentState, diff: DiffLine[]) =>
  workspaceShell({
    ...
    trailing:
      state === "no-session"
        ? ""
        : sidebarPane(sessionInspector(data.inspector), "Session inspector", "inspector"),
    ...
    overlay:
      state === "projects"
        ...
        : state === "chat-search"
          ? workspaceOverlay(chatSearch(data.searchChats, data.projects), "Chat search overlay")
          : "",
  });
export const editorWorkspace = (data: WorkspaceScenario, state: EditorState, code: CodeLine[]) =>
  workspaceShell({
    ...
    drawer: state === "agent" ? workspaceDrawer(conversation(data, true), "conversation") : "",
  });
