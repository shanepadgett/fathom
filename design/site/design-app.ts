...
class DesignApp extends HTMLElement {
  ...
  connectedCallback() {
      ...
      <div data-shell class="min-h-dvh">
        <aside id="design-drawer" data-drawer class="fixed inset-y-0 left-0 z-20 flex w-60 flex-col border-r border-line bg-surface" aria-label="Design navigation">
          <div data-brand class="flex h-20 shrink-0 items-center justify-between gap-2 border-b border-line px-4">
            ...
            <ds-button variant="quiet" size="small" icon-only><button type="button" data-drawer-toggle aria-label="Close navigation" aria-controls="design-drawer" aria-expanded="true">${icon(
              "sidebar-simple",
            )}</button></ds-button>
          </div>
        ...
        <ds-button variant="secondary" data-drawer-open class="viewer-edge-tab fixed left-0 top-1/2 z-30" hidden><button type="button" data-drawer-toggle aria-label="Open navigation" aria-controls="design-drawer" aria-expanded="false">${icon(
          "sidebar-simple",
        )}</button></ds-button>
    ...
    this.addEventListener(
      ...
      (event) => {
        ...
        if (target.closest("[data-drawer-toggle]")) {
          const collapsed = this.querySelector<HTMLElement>("[data-drawer]")!.hidden;
    ...
    const hideEdgeTab = () => {
      this.querySelector("[data-drawer-open]")?.removeAttribute("data-revealed");
    };
  ...
  private renderRoute(focus = false) {
    ...
    let screenActions = "";
    if (path === "/") {
      body = `<div>${this.indexLink("#/tokens", "Tokens")}${this.indexLink(
        "#/components",
        "Components",
      ...
      const entries =
        group === "components" ? components : group === "screens" ? screens : undefined;
      const entry = entries?.find((item) => path === `/${group}/${item.id}`);
      if (entry) {
        title = entry.name;
        description = entry.description;
        if (group === "screens") {
          screenActions = entry.examples
            .map(
              (example, index) =>
        ...
        body = entries.length
          ? `<div>${entries
              .map((item) => this.indexLink(`#/${group}/${item.id}`, item.name))
              .join("")}</div>`
    ...
    content.innerHTML = `
      <header class="mb-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
          ...
          ${screenActions ? `<div class="flex flex-wrap gap-2">${screenActions}</div>` : ""}
        </div>
        ${description ? `<p class="mt-3 text-muted">${description}</p>` : ""}
      </header>
      ${body}`;
    document.title = `${title} — Fathom`;
    for (const link of this.querySelectorAll<HTMLAnchorElement>("[data-nav] a")) {
      if (link.hash === `#${path}`) {
