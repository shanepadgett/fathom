...
class TokensView extends DesignElement {
  ...
  override updated() {
    for (const dot of this.querySelectorAll<HTMLElement>("[data-motion-token]")) {
      ...
      const duration = current
        .getPropertyValue(name.startsWith("--motion-duration-") ? name : "--motion-duration-slow")
        .trim();
      const from = name.startsWith("--motion-scale-") ? `scale(var(${name}))` : "translateX(0)";
      const to = name.startsWith("--motion-scale-")
        ? "scale(1)"
        : `translateX(var(${name.startsWith("--motion-distance-") ? name : "--spacing-16"}))`;
  ...
  override render() {
    ...
    return html`
      <div class="block text-base">
        ...
        ${categories.map(
          (category, index) => html`
            <div id=${`tokens-panel-${index}`} ?hidden=${category !== this.category}>
              ${sections
