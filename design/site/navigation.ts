...
const link = (
  ...
  `<a class="${row}" href="${text(href)}" title="${
    text(fullLabel)
...
const group = (
  ...
  `<details class="viewer-nav-group" ${open ? "open" : ""}>
    <summary class="${row}">${icon(symbol)}<span class="flex-1">${
    text(label)
  }</span><span class="font-mono text-micro text-muted">${entries.length}</span>${
...
export const viewerNavigation = () =>
  `<div class="grid gap-1">${link("#/", "Overview", "folder-open")}${
    link("#/tokens", "Tokens", "brackets-curly")
  }</div>${
    section(
      "Components",
      "#/components",
      (["Primitives", "Composites", "Behavior demos"] as const).map((
        category,
        index,
      ) =>
        group(
