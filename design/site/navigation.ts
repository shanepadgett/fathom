...
const link = (href: string, label: string, fullLabel = label) =>
  html`
    <a class="${row}" href="${href}" title="${fullLabel}"
...
const section = (
  ...
  html`
    <section class="mt-6">
      <h2
        class="mb-2 px-2 text-dense font-semibold uppercase tracking-wide text-ink">
...
export const viewerNavigation = () =>
  html`<div class="grid gap-1">${link("#/", "Overview")}${
    link("#/tokens", "Tokens")
  }</div>
    ${
    section(
