...
export class ProjectPickerElement extends DesignElement {
  ...
  override render() {
    return html`
      ...
      >
        <div
          class="flex items-center gap-2 border-b border-line px-3 py-3 text-muted">
        ...
        <div role="listbox" aria-label="Projects"
          class="max-h-96 overflow-y-auto p-1.5">
              ...
              >
                ${icon("folder")}<span class="flex-1">All projects</span>${icon(
                  "check",
                )}
              </div>
                  ...
                  html`
                    ...
                    >
                      ...
                      <div class="min-w-0 flex-1">
                        <p class="truncate" title=${project.name}>${project
                          .name}</p>
                        <p class="mt-0.5 truncate text-micro text-muted" title=${project
                          .path}>
                                    ${project.path}
                                  </p>
                      </div>
