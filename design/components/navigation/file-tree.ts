...
const treeNode = (node: FileNode, selected: string, expanded: string[]): TemplateResult =>
  node.children
    ? html`<div>
        <folder-item .name=${node.name} .expanded=${expanded.includes(node.id)}></folder-item>${
    ...
    : html` <file-item .file=${node} .selected=${node.id === selected} .tree=${true}></file-item> `;
