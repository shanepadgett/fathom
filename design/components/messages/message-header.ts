...
export class MessageHeaderElement extends DesignElement {
  ...
  override render() {
    ...
    const { message } = this;
    const working = message.agent &&
      (message.working ??
        message.blocks.some((block) =>
          block.kind === "status" && block.tone === "action"
        ));
    return html`
      <header class="mb-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <h3 class="min-w-0 break-words font-semibold ${message.agent
          ? "text-action"
          : ""}">${message.author}</h3>
        ${working
          ? html`<span class="sr-only">Working</span>${
            this.animation
              ? html`
                <span class="identity-motion identity-motion-${this.animation}"
          ...
          : nothing}
        ${message.model
          ? html`<span class="text-sm text-muted">${message.model}</span>`
          : nothing}
        ${!working
          ? html`<span class="text-sm text-muted">${message.time}</span>`
          : nothing}
        ${message.edited
          ? html`<span class="text-sm text-muted">Edited</span>`
          : nothing}
      </header>
    `;
  }
