...
export class MessageAttachmentsElement extends DesignElement {
  ...
  override render() {
    ...
    return html`
      ${
        images.length
          ? html`<ul class="mb-3 flex flex-wrap gap-3" aria-label="Image attachments">
              ${images.map(
                (attachment) => html`
                  <li
      ...
      ${
        files.length || this.annotationCount
          ? html`
              <ul class="mb-4 flex flex-wrap gap-2" aria-label="Attached files and annotations">
                ${files.map(
                  (attachment) =>
                    html`<li class="min-w-0 max-w-full">
                      ${chip(
                        html`${icon("file-text")}<span class="truncate">${attachment.name}</span>`,
                        attachment.detail,
                      )}
                    </li>`,
                )}
                ${
                  this.annotationCount
                    ? html`<li>
                        ${chip(
                          html`${icon("chat-circle-text")}${this.annotationCount}
                          ${this.annotationCount === 1 ? "annotation" : "annotations"}`,
                        )}
                      </li>`
                    : nothing
                }
