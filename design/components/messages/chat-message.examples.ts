...
export const userMessageExamples = [
  {
    name: "Short prompt",
    markup: html`<chat-message
      .item=${messageStructure.user}
      .changes=${noChanges}
    ></chat-message>`,
  },
  {
    ...
    markup: html`<chat-message
      .item=${messageStructure.longUser}
      .changes=${noChanges}
    ></chat-message>`,
  },
  {
    ...
    markup: html`<chat-message
      .item=${{
        ...
        blocks: [
          {
            kind: "prose",
            text: "Use this visual reference and the attached notes to update the conversation.",
          },
        ],
      }}
      .changes=${noChanges}
  ...
  },
  {
    ...
    markup: html`
      <article>
        ...
        ></message-header>
        <streaming-response
          text="The sidebar now reads the shared session title."
        ></streaming-response>
      </article>
    `,
  },
...
export const messageRevisionExamples = [
  {
    ...
    markup: html`
      <div>
        ...
        ></chat-message>
        <div
          class="mt-4 flex flex-wrap items-center gap-2 text-sm text-muted"
          role="group"
    ...
    markup: html`
      <div class="rounded-lg border border-action p-4">
        ...
        <textarea
          ...
          readonly
        >
Also keep the full title visible in the conversation header.</textarea>
      ...
      <article>
        ...
        <blockquote class="mb-4 border-l-2 border-line pl-4 text-muted">
          ...
          <p class="mt-1 text-sm">
            The saved excerpt is still available: “Both views share the session record.”
          </p>
        </blockquote>
        <p>Does that also cover conversations running in the background?</p>
      </article>
...
export const messageDeliveryExamples = [
    ...
    markup: html`
      <div>
        ...
        <p class="mt-3 text-sm">
          <span class="text-muted">Queued · sends after the current run</span>
        </p>
        <div class="mt-3 flex gap-2">
          ${button({
            label: "Edit queued message",
            variant: "quiet",
          })}${button({ label: "Remove from queue", variant: "quiet" })}
        </div>
      </div>
    `,
  },
  ...
  },
  {
    ...
    markup: html`
      <div>
        <chat-message .item=${messageStructure.user} .changes=${noChanges}></chat-message>
        <p class="mt-3 text-sm"><span class="text-danger">Couldn’t send · connection lost</span></p>
  ...
  },
  {
    ...
    markup: html`
      <div>
        <chat-message .item=${messageStructure.user} .changes=${noChanges}></chat-message>
        <p class="mt-3 text-sm"><span class="text-muted">Retrying · message retained</span></p>
        <div class="mt-3">
          ${button({
            label: "Retry sending",
            disabled: true,
