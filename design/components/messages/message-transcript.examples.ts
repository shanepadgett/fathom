...
export const turnGroupingExamples = [
  {
    ...
    markup: html`
      <div class="flex flex-col gap-6">
        ...
        <chat-message .item=${messageStructure
          .update} .changes=${noChanges}></chat-message>
      ...
      <div class="flex flex-col gap-6">
        ...
        <chat-message .item=${{
          ...
          blocks: [{
            kind: "prose",
            text:
              "Both views read the session record. I’ll update the title there, then run the rename checks.",
          }],
        }} .changes=${noChanges}></chat-message>
  ...
  },
  {
    ...
    markup: html`
      <div class="flex flex-col gap-6">
        <chat-message .item=${messageStructure
          .user} .changes=${noChanges}></chat-message>
        <div class="flex items-center gap-3 text-sm text-action" role="separator"
          aria-label="New messages"><span class="h-px flex-1 bg-action/30"></span>New messages<span class="h-px flex-1 bg-action/30"></span></div>
        <chat-message .item=${messageStructure
          .answer} .changes=${noChanges}></chat-message>
      </div>
    `,
  },
  {
    ...
    markup: html`
      <div class="flex flex-col gap-6">
        <chat-message .item=${messageStructure
          .answer} .changes=${noChanges}></chat-message>
        <div class="flex items-center gap-3 text-sm text-muted" role="separator"
          aria-label="Today"><span class="h-px flex-1 bg-line"></span>Today<span class="h-px flex-1 bg-line"></span></div>
        <p
          class="text-center text-sm text-muted">Conversation restored · earlier messages above</p>
        <chat-message .item=${messageStructure
          .followUp} .changes=${noChanges}></chat-message>
      </div>
    `,
