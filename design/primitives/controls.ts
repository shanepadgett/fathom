...
export const button = (
  { label, content, variant = "secondary", size = "compact", disabled = false }:
    ButtonOptions,
) =>
  `<ds-button variant="${variant}" size="${size}"><button type="button" aria-label="${
    text(label)
  }" ${disabled ? "disabled" : ""}>${
    content ?? text(label)
  }</button></ds-button>`;
export const iconButton = (
  ...
  `<ds-button variant="quiet" size="small" icon-only><button type="button" aria-label="${
    text(label)
  }" title="${text(label)}" ${
...
export const projectSelector = (label: string) =>
  `<ds-button variant="secondary" size="project"><button type="button">${
    icon("folder")
  }<span class="flex-1 text-left">${text(label)}</span>${
    icon("caret-down")
  }</button></ds-button>`;
...
export const tabStrip = (items: TabItem[], selected: string, label: string) =>
  `<div class="flex h-9 shrink-0 border-b border-line text-sm" role="group" aria-label="${
    text(label)
...
export const searchFieldPreview = (
  ...
  }<span class="flex min-w-0 items-center" data-search-prompt><span class="h-5 w-px shrink-0 bg-action" aria-hidden="true"></span><span class="truncate">${
    text(placeholder)
  }</span></span>${
