...
export function piPlugin(authPath: string, modelId: string) {
  return definePlugin({
    ...
    activate(ctx) {
      ...
      if (!model) {
        throw new Error(
          `Unknown Codex model ${modelId}. Available: ${
            models.getModels("openai-codex").map((m) => m.id).join(", ")
          }`,
        );
      }
      ctx.provide("model", {
        ...
        async respond(input) {
          ...
          return {
            text: message.content.filter((c) => c.type === "text").map((c) =>
              c.text
            ).join(""),
            calls: message.content.filter((c) => c.type === "toolCall").map(
              (c) => ({ id: c.id, name: c.name, arguments: c.arguments }),
            ),
            stop: message.stopReason === "toolUse"
              ? "tools"
