...
native `deno desktop` CLI using its default WebView backend and V8 engine.
...
```sh
deno task desktop
FATHOM_WORKSPACE="$PWD" ./dist/Fathom.app/Contents/MacOS/laufey_webview
```
...
`deno task dev` runs the browser version with backend watch/restart. Refresh the
page for frontend changes. Changes to the desktop bundle require another build.
...
./dist/Fathom.app/Contents/MacOS/laufey_webview
```
...
[plugin authoring](docs/plugins.md) and [frontend contracts](ui/README.md).
...
ui/core/                   Frontend transport, state, plugin host
ui/plugins/                Replaceable shell, conversation, composition inspector
