...
class Composition extends HostElement {
  ...
  render() {
    ...
    if (!data) return;
    return html`
      <details class="composition-panel" open @toggle=${(event) => {
        ...
        <div class="composition-body">
              ...
              ${repeat(data.plugins, (plugin) => plugin.id, (plugin) =>
                html`
                  <details class="plugin-entry">
                    <summary><span>${plugin
                      .id}</span><span class="plugin-status">${plugin
                      .status}</span></summary>
                    <dl>
                      <dt>PROVIDES</dt>
              ...
              ${repeat(data.tools, (tool) => tool.name, (tool) =>
                html`
                ...
                `)}
              <div class="composition-note">BUILT TO BE REPLACED.<p>${data
                  .profiles?.includes("echo")
                ? "Switch to Echo to try an alternate runtime. Switching starts a fresh session."
                : "This workbench runs your custom composition. Its plugins are selected by configuration."}</p></div>
            </div>
      </details>
    `;
  }
  ...
  activate: (host) =>
    host.registerView("composition", {
      ...
      mount: (container, host) =>
        mountElement("fathom-lit-composition", container, host),
    }),
};
