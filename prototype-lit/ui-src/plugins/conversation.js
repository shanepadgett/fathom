...
class Conversation extends HostElement {
  ...
  resize() {
    ...
    const chrome = parseFloat(style.paddingTop) +
  ...
  async submit(event) {
    ...
    if (
      ...
      this.state.bootstrap?.session.status === "running"
    ) return;
  ...
  message(message) {
    const renderer = [...this.host.renderers.values()].find((entry) =>
  ...
  render() {
    ...
    return html`
      <div
        class="section-heading"><h2>Working transcript</h2><button ?disabled=${busy ||
          !messages.length} @click=${() =>
          this.act("reset")}>New session</button></div>
      <div class="ledger" aria-label="Conversation">
        ${repeat(
          ...
          ${
              guard(
                [JSON.stringify(message), this.revision],
        ...
        ${busy || liveText
          ? html`
            <article
              class="ledger-row live"><span class="row-number">↳</span><div><div class="role">Agent / working</div><pre class="message-text">${liveText ||
                (activeTool
                  ? `${activeTool.name}\n${
                    JSON.stringify(activeTool.args, null, 2)
                  }`
                  : "Thinking…")}</pre></div></article>
          `
          : ""}
      </div>
      <form class="composer" @submit=${this.submit}>
        <textarea id="prompt" aria-label="Your instruction"
          placeholder="Describe the work. Be specific." rows="2" required
          @input=${this.resize} @keydown=${(event) => {
            if ((event.metaKey || event.ctrlKey) && event.key === "Enter") {
              this.querySelector("form").requestSubmit();
            }
          }}></textarea>
        <div
          class="composer-actions"><span class="hint">⌘ / Ctrl + Enter to run</span><button type="button" ?disabled=${!busy} @click=${() =>
            this.act(
              "cancel",
            )}>Stop ■</button><button type="submit" class="primary" ?disabled=${busy ||
            this.submitting}>Run agent ↗</button></div>
      </form>
    `;
  }
}
  ...
  activate: (host) =>
    host.registerView("conversation", {
      ...
      mount: (container, host) =>
        mountElement("fathom-lit-conversation", container, host),
    }),
};
