...
function Message(props) {
  const custom = createMemo(() => {
    ...
    const renderer = [...props.host.renderers.values()].find((entry) =>
    ...
    <Show
      ...
      fallback={
        <Show
          ...
          fallback={<pre class="message-text">{props.message.text || ""}</pre>}
        >
          <details class={`evidence ${props.message.isError ? "failed" : ""}`}>
            <summary>
              {props.message.toolName || "Tool"} /{" "}
              {props.message.isError ? "failed" : "result"}
            </summary>
      ...
      >
        ...
        <Show when={!messages().length}>
          <section class="empty">
            <h3>{emptyTitle}</h3>
            <p>
              Give the agent a task. Inspect its work. Swap the pieces that make
              it yours.
            </p>
            <div class="starters">
              {[
        ...
        <Show when={busy() || state.liveText}>
          <article class="ledger-row live">
            ...
            <div>
              ...
              <pre class="message-text">{state.liveText || (state.activeTool ? `${state.activeTool.name}\n${JSON.stringify(state.activeTool.args, null, 2)}` : "Thinking…")}</pre>
            </div>
          </article>
        </Show>
      </div>
      <form ref={form} class="composer" onSubmit={submit}>
        <textarea
          ref={textarea}
          id="prompt"
  ...
  activate: (host) =>
    host.registerView("conversation", {
      ...
      mount: (container, host) =>
        render(() => <Conversation host={host} />, container),
    }),
};
